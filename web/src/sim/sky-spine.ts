 ...
 * Small support latches are explicitly over-center mechanisms so a brief real
 * contact from the previous macro mechanism can leave the next mechanism free.
 */
export const SKY = {
  z: -36.0,
  baseY: 91.0,
  ...
  pendulumLatchC: 1.1e4,
  pendulumLatchDetentK: 1.0e5,
...
export function ensureSkySpineState(chain: LinkedCascadeState): void {
  addDof(chain, { id: MECH_ID.mc11WindLatch, kind: "linear", q: 0, v: 0, inertia_si: 12, damping_si: 220, min_q: 0, max_q: SKY.windLatchTravelM, stop_restitution: 0 });
  addDof(chain, { id: MECH_ID.mc11Sail, kind: "rotary", q: 0, v: 0, inertia_si: SKY.sailInertiaKgm2, damping_si: SKY.sailDampingNms, min_q: 0, max_q: SKY.sailMaxRad, stop_restitution: 0.02 });
  addDof(chain, { id: MECH_ID.mc11SkyCar, kind: "linear", q: 0, v: 0, inertia_si: SKY.skyCarMassKg, damping_si: 4200, min_q: 0, max_q: SKY.skyCarTravelM, stop_restitution: 0.02 });
  addDof(chain, { id: MECH_ID.mc11Counterweight, kind: "linear", q: 0, v: 0, inertia_si: SKY.counterweightMassKg, damping_si: 3200, min_q: 0, max_q: SKY.skyCarTravelM, stop_restitution: 0.02 });
  addDof(chain, { id: MECH_ID.mc12PendulumLatch, kind: "linear", q: 0, v: 0, inertia_si: 12, damping_si: 220, min_q: 0, max_q: SKY.pendulumLatchTravelM, stop_restitution: 0 });
  addDof(chain, {
    id: MECH_ID.mc12Pendulum,
    kind: "rotary",
    q: SKY.pendulumInitialRad,
    v: 0,
    inertia_si: SKY.pendulumMassKg * SKY.pendulumLengthM ** 2 + SKY.pendulumBallastKg * SKY.pendulumBallastRadiusM ** 2,
    damping_si: SKY.pendulumDampingNms,
    min_q: SKY.pendulumInitialRad,
    max_q: SKY.pendulumMaxRad,
    stop_restitution: 0.03,
  });
  addDof(chain, { id: MECH_ID.mc12Ballast, kind: "linear", q: SKY.pendulumBallastRadiusM, v: 0, inertia_si: SKY.pendulumBallastKg, damping_si: 1.0e4, min_q: 5.5, max_q: 14.0, stop_restitution: 0.04 });
  addDof(chain, { id: MECH_ID.mc12LandingBridge, kind: "linear", q: 0, v: 0, inertia_si: SKY.bridgeMassKg, damping_si: 2.5e4, min_q: 0, max_q: SKY.bridgeTravelM, stop_restitution: 0.02 });
...
export function highAltitudeWindMps(heightM: number): number {
  ...
  return SKY.prevailingWindMps * (0.75 + 0.25 * exposure);
}
...
export function applySkySpineForces(chain: LinkedCascadeState, forces: GeneralizedForces): void {
  ...
  const latchTarget = clamp((mc10Bridge.q - SKY.windLatchBridgeStartM) * SKY.windLatchGain, 0, SKY.windLatchTravelM);
  ...
  const latchDetentTarget = latch.q >= SKY.windLatchOverCenterM ? SKY.windLatchTravelM : 0;
  addGeneralizedForce(forces, MECH_ID.mc11WindLatch, SKY.windLatchDetentK * (latchDetentTarget - latch.q));
  ...
  const pTarget = clamp((car.q - SKY.pendulumTripQ) * SKY.pendulumLatchGain, 0, SKY.pendulumLatchTravelM);
  ...
  const pDetentTarget = pendulumLatch.q >= SKY.pendulumLatchOverCenterM ? SKY.pendulumLatchTravelM : 0;
  addGeneralizedForce(forces, MECH_ID.mc12PendulumLatch, SKY.pendulumLatchDetentK * (pDetentTarget - pendulumLatch.q));

  pendulum.inertia_si = SKY.pendulumMassKg * SKY.pendulumLengthM ** 2 + SKY.pendulumBallastKg * ballast.q ** 2;
  const gravityTorque = -SKY.pendulumMassKg * G * SKY.pendulumLengthM * Math.sin(pendulum.q) - SKY.pendulumBallastKg * G * ballast.q * Math.sin(pendulum.q);
...
export function skySpineFinite(chain: LinkedCascadeState): boolean {
  ensureSkySpineState(chain);
  const ids = [MECH_ID.mc11WindLatch, MECH_ID.mc11Sail, MECH_ID.mc11SkyCar, MECH_ID.mc11Counterweight, MECH_ID.mc12PendulumLatch, MECH_ID.mc12Pendulum, MECH_ID.mc12Ballast, MECH_ID.mc12LandingBridge];
  return ids.every((id) => {
